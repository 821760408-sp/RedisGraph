...
void GraphContext_AddNodeToIndices(GraphContext *gc, Schema *s, Node *n) {
    ...
    bool relevent_index = false;
    uint index_field_count = 0;
    /* For each index in schema, see if node 
     * contains indexed attribute. */
    Index *idx = Schema_GetIndex(s, NULL, IDX_FULLTEXT);
    if(idx) {
        // See if node has a property indexed by index.
        relevent_index = false;
        index_field_count = Index_FieldsCount(idx);
        for(uint i = 0; i < index_field_count; i++) {
            if(GraphEntity_GetProperty((GraphEntity*)n, idx->fields_ids[i]) != PROPERTY_NOTFOUND) {
                relevent_index = true;
                break;
            }
        }
        if(relevent_index) Index_IndexNode(idx, n);
    }
